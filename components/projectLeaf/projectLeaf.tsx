...
export default function ProjectLeaf({ project }: { project: Project }) {
    ...
    return (
        <Link href={`/projects/${project.name}`} className='w-full h-[16rem] p-0'>
            <section className={clsx(styles.card, styles[project.difficulty.toLowerCase()])}>
                ...
                <DifficultyHighlighter difficulty={project.difficulty}>
                    <div className='flex flex-col-reverse items-start w-full m-0 p-0 h-[3rem]'>
                        <h3 className={styles.name}>{project.name}</h3>
                    </div>
                    <div className='flex flex-col items-start w-full m-0 mb-1 p-0 h-[1rem]'>
                        <p className={styles.field}>{project.fields.join(', ')}</p>
                    </div>
                    <div className='flex flex-col justify-center m-0 p-0 h-[6rem]'>
